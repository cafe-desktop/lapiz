...
import os
from gi.repository import Gio, Cdk, Ctk, CtkSource, Lapiz
...
def capture_begin_execute_panel(capture, panel, view, label):
    view.get_window(Ctk.TextWindowType.TEXT).set_cursor(Cdk.Cursor.new(Cdk.CursorType.WATCH))
...
def capture_end_execute_panel(capture, exit_code, panel, view, output_type):
    ...
    view.get_window(Ctk.TextWindowType.TEXT).set_cursor(Cdk.Cursor.new(Cdk.CursorType.XTERM))
