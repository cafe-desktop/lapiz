...
G_DEFINE_TYPE_WITH_PRIVATE (LapizStatusbar, lapiz_statusbar, CTK_TYPE_STATUSBAR)
...
lapiz_statusbar_init (LapizStatusbar *statusbar)
{
	...
	ctk_label_set_width_chars (CTK_LABEL (statusbar->priv->overwrite_mode_label),
	...
	ctk_label_set_width_chars (CTK_LABEL (statusbar->priv->cursor_position_label),
	...
	ctk_frame_set_shadow_type (CTK_FRAME (statusbar->priv->state_frame),
							   CTK_SHADOW_IN);

	hbox = ctk_box_new (CTK_ORIENTATION_HORIZONTAL, 0);
	ctk_container_add (CTK_CONTAINER (statusbar->priv->state_frame), hbox);

	statusbar->priv->load_image = ctk_image_new_from_icon_name ("document-open", CTK_ICON_SIZE_MENU);
	statusbar->priv->save_image = ctk_image_new_from_icon_name ("document-save", CTK_ICON_SIZE_MENU);
	statusbar->priv->print_image = ctk_image_new_from_icon_name ("document-print", CTK_ICON_SIZE_MENU);
	...
	ctk_box_pack_start (CTK_BOX (statusbar),
			    statusbar->priv->state_frame,
			    FALSE, TRUE, 0);

	statusbar->priv->error_frame = ctk_frame_new (NULL);
	ctk_frame_set_shadow_type (CTK_FRAME (statusbar->priv->error_frame), CTK_SHADOW_IN);

	error_image = ctk_image_new_from_icon_name ("dialog-error", CTK_ICON_SIZE_MENU);
	...
	ctk_event_box_set_visible_window  (CTK_EVENT_BOX (statusbar->priv->error_event_box),
					   FALSE);
	ctk_widget_show (statusbar->priv->error_event_box);

	ctk_container_add (CTK_CONTAINER (statusbar->priv->error_frame),
			   statusbar->priv->error_event_box);
	ctk_container_add (CTK_CONTAINER (statusbar->priv->error_event_box),
			   error_image);
	...
	ctk_box_reorder_child (CTK_BOX (statusbar),
			       statusbar->priv->error_frame,
			       0);
}
                               ...
                               gboolean        overwrite)
{
	...
	ctk_label_set_text (CTK_LABEL (statusbar->priv->overwrite_mode_label), msg);
...
lapiz_statusbar_clear_overwrite (LapizStatusbar *statusbar)
{
	...
	ctk_label_set_text (CTK_LABEL (statusbar->priv->overwrite_mode_label), NULL);
}
				     ...
				     gint            col)
{
	...
	ctk_label_set_text (CTK_LABEL (statusbar->priv->cursor_position_label), msg);
...
{
	ctk_statusbar_remove (CTK_STATUSBAR (statusbar),
			       ...
			       const gchar    *format, ...)
{
	...
	statusbar->priv->flash_message_id = ctk_statusbar_push (CTK_STATUSBAR (statusbar),
								context_id,
								msg);
