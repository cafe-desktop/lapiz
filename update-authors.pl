...
=pod
    ...
    update-authors.pl is part of Lapiz.
    ...
    along with Lapiz.  If not, see <http://www.gnu.org/licenses/>.
...
sub GetCurrentAuthors {
  my @authors;
  open(FILE,"lapiz/lapiz.about") or die "Can't open lapiz/lapiz.about";
  while (<FILE>) {
    if (/^Authors=*(.+)$/) {
      @authors=split(";",$1);
    }
